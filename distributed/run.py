...
def run_managed(accelerator, devices, main_single):
    ...
    if accelerator == "gpu":
        ...
        if is_custom_managed_run() or is_mpi_managed_run():
...
def run_single_or_multiprocess(accelerator, devices, main_single, master_port, mig_devices):
    ...
    if world_size == 1:
        ...
        logging.info(f"running multi process training on {world_size} processes (devices={devices})")
...
def _check_single_device_visible():
    ...
    assert visible_device_count <= 1, f"{visible_device_count} visible devices ({os.environ['CUDA_VISIBLE_DEVICES']})"
